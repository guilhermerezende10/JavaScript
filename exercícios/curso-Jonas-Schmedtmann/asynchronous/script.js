...
// get3Countries('brazil', 'thailand', 'norway');
...
(async function () {
  const res = await Promise.race([
    ...
    getJSON(`https://restcountries.com/v3.1/name/singapore`),
  ]);
  // console.log(res[0]);
})();
...
const timeout = function (sec) {
  return new Promise(function (_, reject) {
    setTimeout(function () {
      reject(new Error('Requeste took too long!'));
    }, sec * 1000);
  });
};
...
// Promise.allSettled
Promise.allSettled([ // Returns all promises, even the rejected ones
  Promise.resolve('Success'),
  Promise.reject('ERROR'),
  Promise.resolve('Another Success'),
]).then(res => console.log(res));

Promise.all([ // If there is a rejected promised, it will return a error
  Promise.resolve('Success'),
  Promise.reject('ERROR'),
  Promise.resolve('Another Success'),
])
  ...
  // Promise.any [ES2021] // rejected promises will be ignored
